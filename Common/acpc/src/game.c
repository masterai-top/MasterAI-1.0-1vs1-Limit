...
static char actingPlayerChars[ MAX_PLAYERS + 1 ] = "0123456789";
...
void LogGame(const Game *game, char *logBuf, int len)
{
    ...
    {
        snprintf(szStack + strlen(szStack), sizeof(szStack) - 1 - strlen(szStack), "%d ", game->stack[i]);
        snprintf(szBlind + strlen(szBlind), sizeof(szBlind) - 1 - strlen(szBlind), "%d ", game->blind[i]);
    }

    char szRaiseSize[64]    = {0};
    char szFirstPlayer[64]  = {0};
    char szMaxRaises[64]    = {0};
    char szNumBoardCards[64]= {0};
    for(uint8_t i = 0; i < game->numRounds; i++)
    {
        snprintf(szRaiseSize + strlen(szRaiseSize), sizeof(szRaiseSize) - 1 - strlen(szRaiseSize), "%d ", game->raiseSize[i]);
        snprintf(szFirstPlayer + strlen(szFirstPlayer), sizeof(szFirstPlayer) - 1 - strlen(szFirstPlayer), "%d ", game->firstPlayer[i]);
        snprintf(szMaxRaises + strlen(szMaxRaises), sizeof(szMaxRaises) - 1 - strlen(szMaxRaises), "%d ", game->maxRaises[i]);
        snprintf(szNumBoardCards + strlen(szNumBoardCards), sizeof(szNumBoardCards) - 1 - strlen(szNumBoardCards), "%d ", game->numBoardCards[i]);
    }

    snprintf(logBuf, len - 1, 
        "bettingType=%d| numPlayers=%d| stack=%s| blind=%s| numRounds=%d| numSuits=%d| numRanks=%d| numHoleCards=%d| raiseSize=%s| firstPlayer=%s| maxRaise=%s| numBoardCards=%s", 
        game->bettingType, game->numPlayers, szStack, szBlind, game->numRounds, game->numSuits, game->numRanks, game->numHoleCards, szRaiseSize, szFirstPlayer, szMaxRaises, szNumBoardCards);
}
...
uint8_t currentPlayer( const Game *game, const State *state )
{
    ...
    return nextPlayer( game, state, game->firstPlayer[ state->round ] + game->numPlayers - 1 );
}
...
int raiseIsValid( const Game *game, const State *curState, int32_t *minSize, int32_t *maxSize )
{
    int p;
    ...
    if( curState->numActions[ curState->round ] + game->numPlayers > MAX_NUM_ACTIONS ) 
    {
...
{
    int p = 0;
    if(stateFinished(curState)) 
    {
        return 0;
    }

    p = currentPlayer( game, curState );
    if(curState->spent[p] == curState->maxSpent || curState->spent[p] == game->stack[p]) 
    {
...
int isValidAction( const Game *game, const State *curState, const int tryFixing, Action *action )
{
    int min, max, p;
    ...
    {
        // not all-in state, allow fold
        //if( curState->spent[ p ] == curState->maxSpent || curState->spent[ p ] == game->stack[ p ] ) 
        if( curState->spent[ p ] == game->stack[ p ] ) 
        {
            /* player has already called all bets, or is all-in */
            return 0;
        }
...
void doAction( const Game *game, const Action *action, State *state, bool *bCurRoundEnd)
{
    ...
    *bCurRoundEnd = false;
    ...
    case a_call:
        ...
        state->action[ state->round ][ state->numActions[ state->round ] - 1].size = state->spent[ p ];
        break;
    ...
    if(0 == state->round)
    {
        state->roundSpent[0] = state->maxSpent;
    }
    else if(a_raise == action->type)
    {    
        state->roundSpent[state->round] = state->maxSpent;
        for(uint8_t i = 0; i < state->round; i++)
        {
            state->roundSpent[state->round] -= state->roundSpent[i];
        }
    }
...
double valueOfState( const Game *game, const State *state, const uint8_t player )
{
    ...
    while( 1 ) 
    {
        ...
        if( rank[ playerIdx ] == winRank ) 
        {
            ...
            value += (double)( size * ( numPlayers - numWinners ) ) / (double)numWinners;
        } 
        else 
        {
...
static int readBetting( const char *string, const Game *game, State *state )
{
  ...
  while( 1 ) {
    ...
    bool bRoundEnd;
    doAction( game, &action, state, &bRoundEnd);
    c += r;
  }
...
int printBetting2( const Game *game, const State *state, const int maxLen, char *string, char cDelimiter)
{
    ...
    for( i = 0; i <= state->round; ++i ) 
    {
        ...
        {
            r = printAction( game, &state->action[ i ][ a ], maxLen - c, &string[ c ] );
            if( r < 0 ) {
	            return -1;
            }
            c += r;
				 ...
				 const int maxLen, char *string )
{
    ...
    for( p = 0; p < game->numPlayers; ++p ) 
    {
        ...
        r = printCards( game->numHoleCards, state->holeCards[ p ], maxLen - c, &string[ c ] );
        if( r < 0 ) 
        {
            return -1;
        }
...
static int printAllBoardCards(const Game *game, const State *state, const int maxLen, char *string)
{
    int i, c, r;
    ...
    for( i = 0; i < game->numRounds; ++i ) 
    {
        ...
        r = printCards( game->numBoardCards[ i ], &state->boardCards[ bcStart( game, i ) ], maxLen - c, &string[ c ] );
        if( r < 0 ) 
        {
            return -1;
        }
...
int printSpent(const Game *game, const State *state, const int maxLen, char *string)
{
    int c, r;
    ...
    {
        r = snprintf( &string[ c ], maxLen - c, "%" PRId32, state->spent[i]);
        if( r < 0 ) 
        {
           return -1;
        }
...
int printActingPlayerAction( const Game *game, const Action *action, uint8_t acting_player, const int maxLen, char *string )
{
    int c, r;
    ...
    //example: [0r200] or [0f] or [0c]
    c = 0;
    string[c]     = '[';
    string[c + 1] = actingPlayerChars[acting_player];
    string[c + 2] = actionChars[ action->type ];
    c += 3;
...
int printDetailedBetting( const Game *game, const State *state, const int maxLen, char *string )
{
    ...
    for( i = 0; i <= state->round; ++i ) 
    {
        ...
        for( a = 0; a < state->numActions[ i ]; ++a ) {
            ...
            r = printActingPlayerAction(game, &state->action[ i ][ a ], state->actingPlayer[i][a], maxLen - c, &string[ c ] );
            if( r < 0 ) {
                return -1;
            }
            c += r;
...
static int printStateCommonDetailed( const Game *game, const State *state, const int maxLen, char *string )
{
    int c, r;
    ...
    /* HEADER:handId:Detailedbetting (Detailedbetting=actingpalyer-betting) */ 
    r = printDetailedBetting( game, state, maxLen - c, &string[ c ] );
    if( r < 0 ) {
        return -1;
    }
    c += r;
...
int printStateDetailed(const Game *game, const State *state, const int maxLen, char *string)
{
    ...
    /* STATE:handId:betting: */
    r = printStateCommonDetailed( game, state, maxLen - c, &string[ c ] );
    if( r < 0 ) {
        return -1;
    }
    c += r;
    ...
    /* STATE:handId:betting:holeCards boardCards */
    r = printAllBoardCards( game, state, maxLen - c, &string[ c ] );
    if( r < 0 ) {
        return -1;
    }
    c += r;

    r = printSpent(game, state, maxLen - c, &string[c]);
    if( r < 0 ) {
        return -1;
    }
    c += r;
...
int printAction( const Game *game, const Action *action, const int maxLen, char *string )
{
  int c, r;
...
//转换python格式牌的字符串, 例: Ac-2s-Qd-Th-Ks-Dd-3d  (先公牌,后私牌)
int toPythonCards(const Game *game, const State *state, uint8_t player, char *cards, int len)
{
    ...
    //公牌
    cnt = sumBoardCards(game, state->round);
    for( i = 0; i < cnt; ++i ) 
    {
        if(i > 0) {
            cards[c] = '-';
        ...
        r = printCard(state->boardCards[i], len - c,  cards + c);
		if(r < 0) {
            return -1;
        }
    ...
    for(i = 0; i < game->numHoleCards; i++)
    {
        ...
        r =  printCard(state->holeCards[player][i], len - c, cards + c);
        if(r < 0) {
            return -12;
        }
...
int printAllCards(const Game *game, const State *state, char *cards)
{
    ...
    for(i = 0; i < game->numPlayers; i++)
    {
        for(j = 0; j < game->numHoleCards; j++)
        {
            len += sprintf(cards + len, "%d, ", state->holeCards[i][j]);
        }
    }

    //公牌
    i = sumBoardCards(game, game->numRounds);
    for(j = 0; j < i; j++) 
    {
        len += sprintf(cards + len, "%d, ", state->boardCards[j]);
    }
