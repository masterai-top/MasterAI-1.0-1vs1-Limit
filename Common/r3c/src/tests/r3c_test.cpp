...
static void test_stream(const std::string& redis_cluster_nodes, const std::string& redis_password);
...
int main(int argc, char* argv[])
{
    ...
    test_stream(redis_cluster_nodes, redis_password);
    return 0;
    ////////////////////////////////////////////////////////////////////////////
    // EVAL
...
void test_incrby(const std::string& redis_cluster_nodes, const std::string& redis_password)
{
    ...
    try
    {
        ...
        // Test:
        // incrby(key, increment, expired_increment, expired_seconds, which, retry_times)
        // 此重载版本的incrby，只有incrby返回的值等于expired_increment时，才会设置过期�?        n = rc.incrby(key, 2000, 2018, 2);
        if (n != 2000)
        {
...
void test_stream(const std::string& redis_cluster_nodes, const std::string& redis_password)
{
    TIPS_PRINT();

    try
    {
        r3c::CRedisClient rc(redis_cluster_nodes, 2000, -1, redis_password);
        std::vector<r3c::StreamEntry> values;
        do
        {
            rc.xread("channel", 1, 0, &values);
            SUCCESS_PRINT("read msg:%s from channel", values[0].id.c_str());
            for(size_t i=0; i<values[0].fvpairs.size(); i++)
            {
                SUCCESS_PRINT("field:%s-->value:%s", values[0].fvpairs[i].field.c_str(), values[0].fvpairs[i].value.c_str());
            }
        }while(1);
    }
    catch (r3c::CRedisException& ex)
