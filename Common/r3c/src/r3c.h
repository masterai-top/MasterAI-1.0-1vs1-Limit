...
// Redis 命令参数
// 调用final函数前,需要将command&key都add_arg.  此时 _argv[0]=command, _argv[1]=key
class CommandArgs
{
public:
    CommandArgs();
...
public: // KV
    ...
    // Time complexity: O(1)
    // 当过期时间移除成功时，返回true, 如果key不存在或key没有设置过期时间,返回false .
    bool persist(const std::string& key, Node* which=NULL, int num_retries=NUM_RETRIES);
    ...
    // Time complexity: O(1)
    int set(const std::string& key, const std::string& value, Node* which=NULL, int num_retries=NUM_RETRIES);
