...
private:
    CRedisMasterNode* _redis_master_node;   //unused
};
...
int64_t CRedisClient::dbsize()
{
    const int num_retries = NUM_RETRIES;
    const std::string key;
    
    CommandArgs cmd_args;
    cmd_args.add_arg("DBSIZE");
    cmd_args.final();

    // Bulk string reply:
    // the score of member (a double precision floating point number), represented as string.
    // If member does not exist in the sorted set, or key does not exist, nil is returned.
    const RedisReplyHelper redis_reply = redis_command(true, num_retries, key, cmd_args, NULL);
    if(NULL == redis_reply.get()) 
    {
        //错误
        return -1;
    }
...
bool CRedisClient::persist(const std::string& key, Node* which, int num_retries)
{
    CommandArgs cmd_args;
    cmd_args.add_arg("PERSIST");
    cmd_args.add_arg(key);
    cmd_args.final();
        ...
        int num_retries)
{
    ...
    if (REDIS_REPLY_STATUS == redis_reply->type && 0 == strcmp(redis_reply->str, "OK"))
    {
        return 0;
    }
        ...
        int num_retries)
{
    ...
    if(NULL == redis_reply.get())
        return false;
    if (REDIS_REPLY_INTEGER == redis_reply->type)
        return (1 == redis_reply->integer || 0 == redis_reply->integer);
    return true;
}
...
void CRedisClient::init()
{
    try
    {
        LOG(LT_INFO, "r3c::client init start ...");
...
bool CRedisClient::init_standlone(struct ErrorInfo* errinfo)
{
    ...
    {
        LOG(LT_ERROR, "r3c::client init start| msg=%s", errinfo->errmsg.c_str());
        return false;
    }
    else
    {
